...
| count | type为textarea模式下显示计数 | Boolean | false | true | 0.3.7 |
...
| confirm-hold | 点击键盘右下角完成按钮时，是否保持键盘不收起 | Boolean | false | true | 0.6.7 |

### Events

| 事件名 | 说明 | 回调参数 | 版本 |
|--------|------|---------|------|
| input | 输入内容变化时触发 | value: 当前输入框内容 | - |
| blur | 输入框失焦时触发 | value: 当前输入框内容 | - |
| focus | 输入框聚焦时触发 | value: 当前输入框内容 | - |
| confirm | 点击键盘右下角完成按钮时触发 | value: 当前输入框内容 | - |
| click | 点击输入框时触发 | - | - |
| clear | 点击清除按钮时触发 | - | 0.6.9 |
