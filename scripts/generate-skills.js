...
function generateFrontmatter(name, description, type, url) {
    ...
    switch (type) {
        case 'component':
            invokeText = `Invoke when user needs to use ${name} component in their uni-app project.`;
            break;
        case 'tool':
            invokeText = `Invoke when user needs to use ${name} tool in their uni-app project.`;
            break;
        case 'hook':
            invokeText = `Invoke when user needs to use ${name} hook in their uni-app project.`;
            break;
        case 'guide':
            invokeText = `Invoke when user needs guidance on ${name} in uView Pro.`;
            break;
        case 'layout':
            invokeText = `Invoke when user needs layout examples for ${name} in uView Pro.`;
            break;
        default:
...
function createSkillsReadme() {
    const readmeContent = `---
name: "skills"
description: "uView Pro 组件库的所有技能文档，包含指南、组件、工具、钩子和布局的使用说明。"
---
...
- [http](references/tools/http.md) - HTTP工具
...
- [request](references/tools/request.md) - 请求工具
...
1. 查看技能文档：在 references 目录下找到对应的技能文档
2. 了解组件用法：每个技能文档包含详细的使用说明、API文档和示例代码
3. 集成到项目：根据文档中的示例代码，将组件、工具或钩子集成到您的uni-app项目中
